...
class RepoListAdapter(private val repositoryClicked: (repository: Repository) -> Unit) : RecyclerView.Adapter<RepoItemViewHolder>() {

    private var repos: List<Repository> = emptyList()
    private val unsubscribe: PublishSubject<Any> = PublishSubject.create()

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RepoItemViewHolder = RepoItemViewHolder(RepositoryRowBinding.inflate(LayoutInflater.from(parent.context)))
    ...
    override fun onBindViewHolder(holder: RepoItemViewHolder, position: Int) {
        ...
        RxView.clicks(holder.binding.root).takeUntil(unsubscribe).subscribe { repositoryClicked(repo) }
