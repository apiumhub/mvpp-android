...
interface RepositoryDetailsRepository {
  ...
  companion object {
    fun create(): RepositoryDetailsRepository =
      InMemoryRepositoryDetailsRepository()
...
class InMemoryRepositoryDetailsRepository : InMemoryRepository(),
  RepositoryDetailsRepository {
  private var repositoryDetails: RepositoryDetailsDto =
    RepositoryDetailsDto(null, null)
  ...
  override fun getRepositoryDetails(): Observable<RepositoryDetailsDto> =
    Observable.just(if (!isExpired) repositoryDetails else RepositoryDetailsDto(
      null,
      null
    )
    )
